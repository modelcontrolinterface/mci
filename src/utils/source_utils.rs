...
impl DefinitionSource {
    ...
    fn parse_url(input: &str) -> Result<Self, AppError> {
        let url = Url::parse(input).map_err(|_| AppError::invalid_definition_source(input))?;
    ...
    fn parse_file_url(url: Url, input: &str) -> Result<Self, AppError> {
        let path = url.to_file_path().map_err(|()| {
            AppError::invalid_definition_source(format!(
    ...
    fn parse_file_path(input: &str) -> Result<Self, AppError> {
        ...
        if path.components().count() == 0 {
            return Err(AppError::invalid_definition_source(input));
        }
...
mod test {
    ...
    fn test_file_url_valid() {
        ...
        let result = DefinitionSource::parse(file_url.as_str());

        assert!(result.is_ok());
        match result.unwrap() {
            DefinitionSource::File(path) => assert_eq!(path, file_path),
    ...
    fn test_file_url_not_found() {
        let result = DefinitionSource::parse("file:///nonexistent/path/definition.json");
    ...
    fn test_absolute_path_valid() {
        ...
        let result = DefinitionSource::parse(file_path.to_str().unwrap());
        assert!(result.is_ok());
    ...
    fn test_file_path_not_found() {
        let result = DefinitionSource::parse("/nonexistent/path/definition.json");
        assert!(result.is_err());
    ...
    fn test_file_path_is_directory() {
        ...
        let result = DefinitionSource::parse(temp_dir.path().to_str().unwrap());
        assert!(result.is_err());
    ...
    fn test_empty_string_returns_error() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            AppError::InvalidDefinitionSource(_)
        ));
    }
    ...
    fn test_as_path_helper() {
        ...
        let source = DefinitionSource::parse(file_path.to_str().unwrap()).unwrap();
        assert!(source.as_path().is_some());
        assert_eq!(source.as_path().unwrap(), file_path.as_path());

        let http_source = DefinitionSource::parse("http://example.com/def.json").unwrap();
    ...
    fn test_as_url_helper() {
        ...
        let file_source = DefinitionSource::parse(file_path.to_str().unwrap()).unwrap();
